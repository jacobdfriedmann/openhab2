...
/**
 * ZWave controller class. Implements communication with the Z-Wave controller
...
public class ZWaveController {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ZWaveController.class);

	private static final int ZWAVE_RESPONSE_TIMEOUT = 5000; // 5000 ms
															// ZWAVE_RESPONSE
															// TIMEOUT
	private static final int ZWAVE_RECEIVE_TIMEOUT = 1000; // 1000 ms
															// ZWAVE_RECEIVE_TIMEOUT
	private static final int INITIAL_TX_QUEUE_SIZE = 128;
	private static final int INITIAL_RX_QUEUE_SIZE = 8;
	private static final long WATCHDOG_TIMER_PERIOD = 10000; // 10 seconds
																// watchdog
																// timer
	...
	private final PriorityBlockingQueue<SerialMessage> sendQueue = new PriorityBlockingQueue<SerialMessage>(
			INITIAL_TX_QUEUE_SIZE, new SerialMessage.SerialMessageComparator(
					this));
	private final PriorityBlockingQueue<SerialMessage> recvQueue = new PriorityBlockingQueue<SerialMessage>(
			INITIAL_RX_QUEUE_SIZE, new SerialMessage.SerialMessageComparator(
					this));
	 ...
	 *            the serial port name to use for communication with the Z-Wave
	 *            controller stick.
	 * @throws SerialInterfaceException
	 *             when a connection error occurs.
	 */
	public ZWaveController(final boolean masterController, final boolean isSUC,
			final String serialPortName, final Integer timeout,
			final boolean reset) throws SerialInterfaceException {
		...
		logger.info("Z-Wave timeout is set to {}ms. Soft reset is {}.",
				zWaveResponseTimeout, reset);
		connect(serialPortName);
		this.watchdog = new Timer(true);
		this.watchdog.schedule(new WatchDogTimerTask(serialPortName),
				WATCHDOG_TIMER_PERIOD, WATCHDOG_TIMER_PERIOD);

		// We have a delay in running the initialisation sequence to allow any
		// frames queued in the controller to be received before sending the
		// init
	...
	private class InitializeDelayTask extends TimerTask {
		private final Logger logger = LoggerFactory
				.getLogger(WatchDogTimerTask.class);
	 ...
	 * that are a response to our own requests, or the stick asking us
	 * information.
	 * 
	...
	private void handleIncomingMessage(SerialMessage incomingMessage) {
		...
		case Response:
			...
			break;
		default:
			logger.warn("Unsupported incomingMessageType: {}",
	...
	private void handleIncomingRequestMessage(SerialMessage incomingMessage) {
		...
		ZWaveCommandProcessor processor = ZWaveCommandProcessor
				.getMessageDispatcher(incomingMessage.getMessageClass());
		if (processor == null) {
			logger.warn(String
					.format("TODO: Implement processing of Request Message = %s (0x%02X)",
		...
		boolean result = processor.handleRequest(this, lastSentMessage,
				incomingMessage);
		if (processor.isTransactionComplete()) {
			notifyEventListeners(new ZWaveTransactionCompletedEvent(
					this.lastSentMessage, result));
	...
	private void handleFailedSendDataRequest(SerialMessage originalMessage) {
		new SendDataMessageClass().handleFailedSendDataRequest(this,
				originalMessage);
	}
	...
	private void handleIncomingResponseMessage(SerialMessage incomingMessage) {
		...
		if (processor == null) {
			logger.warn(String
					.format("TODO: Implement processing of Response Message = %s (0x%02X)",
							incomingMessage.getMessageClass().getLabel(),
							incomingMessage.getMessageClass().getKey()));
		...
		boolean result = processor.handleResponse(this, lastSentMessage,
				incomingMessage);
		if (processor.isTransactionComplete()) {
			notifyEventListeners(new ZWaveTransactionCompletedEvent(
					this.lastSentMessage, result));
		...
		case GetVersion:
			this.zWaveVersion = ((GetVersionMessageClass) processor)
					.getVersion();
			this.ZWaveLibraryType = ((GetVersionMessageClass) processor)
					.getLibraryType();
			break;
		case MemoryGetId:
		...
		case SerialApiGetInitData:
			...
			for (Integer nodeId : ((SerialApiGetInitDataMessageClass) processor)
					.getNodes()) {
				// if(nodeId != 35)
				// continue;
				...
				try {
					...
					logger.error("NODE {}: Error deserialising XML file. {}",
							nodeId, e.toString());
					node = null;
				}
				String name = null;
				...
				if (node != null) {
					// Remember the name and location - in case we decide the
					// file was invalid
					name = node.getName();
					...
					if (node.getManufacturer() == Integer.MAX_VALUE
						...
						logger.warn(
								"NODE {}: Config file data is invalid, ignoring config.",
								nodeId);
						node = null;
					} else {
						// The restore was ok, but we have some work to set up
						// the links that aren't
						...
						// Set the controller and node references for all
						// command classes
						for (ZWaveCommandClass commandClass : node
								.getCommandClasses()) {
							...
							// If this is the multi-instance class, add all
							// command classes for the endpoints
							if (commandClass instanceof ZWaveMultiInstanceCommandClass) {
								for (ZWaveEndpoint endPoint : ((ZWaveMultiInstanceCommandClass) commandClass)
										.getEndpoints()) {
									for (ZWaveCommandClass endpointCommandClass : endPoint
											.getCommandClasses()) {
										endpointCommandClass
												.setController(this);
				...
				if (node == null) {
					...
					// Try to maintain the name and location (user supplied
					// data)
					// even if the XML file was considered corrupt and we reload
				...
				if (nodeId == this.ownNodeId) {
					...
					// We already know the device type, id, manufacturer so set
					// it here
		...
		case GetSucNodeId:
			...
			if (this.setSUC == true && this.sucID == 0) {
				// We want to be SUC
				this.enqueue(new EnableSucMessageClass()
						.doRequest(EnableSucMessageClass.SUCType.SERVER));
				this.enqueue(new SetSucNodeMessageClass().doRequest(
						this.ownNodeId, SetSucNodeMessageClass.SUCType.SERVER));
				...
				this.enqueue(new EnableSucMessageClass()
						.doRequest(EnableSucMessageClass.SUCType.NONE));
				this.enqueue(new SetSucNodeMessageClass().doRequest(
						this.ownNodeId, SetSucNodeMessageClass.SUCType.NONE));
			}
			this.enqueue(new GetControllerCapabilitiesMessageClass()
					.doRequest());
			break;
		case SerialApiGetCapabilities:
			this.serialAPIVersion = ((SerialApiGetCapabilitiesMessageClass) processor)
					.getSerialAPIVersion();
			this.manufactureId = ((SerialApiGetCapabilitiesMessageClass) processor)
					.getManufactureId();
			this.deviceId = ((SerialApiGetCapabilitiesMessageClass) processor)
					.getDeviceId();
			this.deviceType = ((SerialApiGetCapabilitiesMessageClass) processor)
					.getDeviceType();

			this.enqueue(new SerialApiGetInitDataMessageClass().doRequest());
			break;
		case GetControllerCapabilities:
			this.controllerType = ((GetControllerCapabilitiesMessageClass) processor)
					.getDeviceType();
			break;
		default:
			break;
	...
	public void connect(final String serialPortName)
		...
		try {
			...
			CommPort commPort = portIdentifier.open(
					"org.openhab.binding.zwave", 2000);
			...
			throw new SerialInterfaceException(String.format(
					"Port %s does not exist", serialPortName), e);
		} catch (PortInUseException e) {
			logger.error("Serial Error: Port {} in use.", serialPortName);
			throw new SerialInterfaceException(String.format("Port %s in use.",
					serialPortName), e);
			...
			logger.error(
					...
					serialPortName);
			throw new SerialInterfaceException(String.format(
					"Unsupported comm operation on Port %s.", serialPortName),
					e);
		} catch (TooManyListenersException e) {
			logger.error("Serial Error: Too many listeners on Port {}.",
					serialPortName);
			e.printStackTrace();
		}
	}
	...
	public void close() {
		...
		ArrayList<ZWaveEventListener> copy = new ArrayList<ZWaveEventListener>(
				this.zwaveEventListeners);
	...
	public void disconnect() {
		...
		if (transactionCompleted.availablePermits() < 0) {
			transactionCompleted.release(transactionCompleted
					.availablePermits());
		}

		transactionCompleted.drainPermits();
		logger.trace("Transaction completed permit count -> {}",
				transactionCompleted.availablePermits());
	...
	private class ZWaveInitNodeThread extends Thread {
		...
		public void run() {
			...
			if (zwaveNodes.get(nodeId) != null) {
				logger.warn(
						"NODE {}: Attempting to add node that already exists",
						nodeId);
				return;
			}
			...
			try {
				...
				logger.error(
						"NODE {}: Restore from config: Error deserialising XML file. {}",
						nodeId, e.toString());
				node = null;
			}
			String name = null;
			...
			// Did the node deserialise ok?
			if (node != null) {
				// Remember the name and location - in case we decide the file
				// was invalid
				name = node.getName();
				location = node.getLocation();
				...
				if (node.getManufacturer() == Integer.MAX_VALUE
						...
						|| node.getNodeId() != nodeId) {
					logger.warn(
							"NODE {}: Restore from config: Error. Data invalid, ignoring config.",
							nodeId);
					node = null;
				} else {
					...
					for (ZWaveCommandClass commandClass : node
						...
						if (commandClass instanceof ZWaveEventListener) {
							controller
									.addEventListener((ZWaveEventListener) commandClass);
						}
						...
						if (commandClass instanceof ZWaveMultiInstanceCommandClass) {
							for (ZWaveEndpoint endPoint : ((ZWaveMultiInstanceCommandClass) commandClass)
								...
								for (ZWaveCommandClass endpointCommandClass : endPoint
									...
									if (endpointCommandClass instanceof ZWaveEventListener) {
										controller
												.addEventListener((ZWaveEventListener) endpointCommandClass);
									}
								}
							}
						}
	...
	public void enqueue(SerialMessage serialMessage) {
		...
		if (node != null) {
			...
			if (!node.isListening() && !node.isFrequentlyListening()) {
				...
				if (wakeUpCommandClass != null
						&& !wakeUpCommandClass
								.processOutgoingWakeupMessage(serialMessage)) {
					return;
				}
			}
		}
		...
		logger.debug("Enqueueing message. Queue length = {}",
				this.sendQueue.size());
	}
	...
	public void notifyEventListeners(ZWaveEvent event) {
		logger.debug("Notifying event listeners: {}", event.getClass()
				.getSimpleName());
		ArrayList<ZWaveEventListener> copy = new ArrayList<ZWaveEventListener>(
				this.zwaveEventListeners);
		...
		if (event instanceof ZWaveInclusionEvent) {
			...
			case IncludeDone:
				...
				if (getNode(incEvent.getNodeId()) != null) {
					logger.debug(
							"NODE {}: Newly included node already exists - not initialising.",
							incEvent.getNodeId());
					break;
				}
			...
			case DeleteNode:
				if (getNode(networkEvent.getNodeId()) == null) {
					logger.debug(
							"NODE {}: Deleting a node that doesn't exist.",
							networkEvent.getNodeId());
					break;
				}
			...
			logger.debug("NODE {}: Node Status event - Node is {}",
					statusEvent.getNodeId(), statusEvent.getState());

			// Get the node
			ZWaveNode node = getNode(event.getNodeId());
			if (node == null) {
				logger.error("NODE {}: Node is unknown!",
	...
	public void pollNode(ZWaveNode node) {
		for (ZWaveCommandClass zwaveCommandClass : node.getCommandClasses()) {
			logger.trace("NODE {}: Inspecting command class {}", node
					.getNodeId(), zwaveCommandClass.getCommandClass()
					.getLabel());
			if (zwaveCommandClass instanceof ZWaveCommandClassDynamicState) {
				logger.debug("NODE {}: Found dynamic state command class {}",
						node.getNodeId(), zwaveCommandClass.getCommandClass()
								.getLabel());
				...
				if (instances == 1) {
					Collection<SerialMessage> dynamicQueries = zdds
				...
				for (ZWaveEndpoint endpoint : multiInstanceCommandClass
						.getEndpoints()) {
					for (ZWaveCommandClass endpointCommandClass : endpoint
							.getCommandClasses()) {
						logger.trace(
								"NODE {}: Inspecting command class {} for endpoint {}",
								node.getNodeId(), endpointCommandClass
										.getCommandClass().getLabel(), endpoint
										.getEndpointId());
						if (endpointCommandClass instanceof ZWaveCommandClassDynamicState) {
							logger.debug(
									"NODE {}: Found dynamic state command class {}",
									node.getNodeId(), endpointCommandClass
											.getCommandClass().getLabel());
							ZWaveCommandClassDynamicState zdds2 = (ZWaveCommandClassDynamicState) endpointCommandClass;
							Collection<SerialMessage> dynamicQueries = zdds2
									.getDynamicValues(true);
							for (SerialMessage serialMessage : dynamicQueries) {
								sendData(node.encapsulate(serialMessage,
										endpointCommandClass,
	...
	public void requestNodeNeighborUpdate(int nodeId) {
		this.enqueue(new RequestNodeNeighborUpdateMessageClass()
				.doRequest(nodeId));
	}
	 ...
	 * Therefore, the number of retries is set to 1. NOTE: On some (most!)
	 * ZWave-Plus sticks, this can cause the stick to hang.
	 */
	...
	public void requestAssignReturnRoute(int nodeId, int destinationId) {
		this.enqueue(new AssignReturnRouteMessageClass().doRequest(nodeId,
				destinationId, getCallbackId()));
	}
	...
	/**
	 * Transmits the SerialMessage to a single Z-Wave Node. Sets the
	 * transmission options as well.
	 * 
	 * @param serialMessage
	...
	public void sendData(SerialMessage serialMessage) {
		...
		if (serialMessage.getMessageClass() != SerialMessageClass.SendData) {
			logger.error(String.format(
					"Invalid message class %s (0x%02X) for sendData",
					serialMessage.getMessageClass().getLabel(), serialMessage
							.getMessageClass().getKey()));
			return;
		}
		...
		// We need to wait on the ACK from the controller before completing the
		// transaction.
		// This is required in case the Application Message is received from the
		// SendData ACK
		serialMessage.setAckRequired();

		serialMessage.setTransmitOptions(TRANSMIT_OPTION_ACK
	...
	/**
	 * Indicates a working connection to the Z-Wave controller stick and
	...
	/**
	 * Input thread. This processes incoming messages - it decouples the receive
	 * thread, which responds to messages from the controller, and the actual
	 * processing of messages to ensure we respond to the controller in a timely
	 * manner
	 * 
	 * @author Chris Jackson
	 */
		...
		public void run() {
			...
			while (!interrupted()) {
				try {
					...
					logger.debug("Receive queue TAKE: Length={}",
							recvQueue.size());
					logger.debug("Process Message = {}", SerialMessage
							.bb2hex(recvMessage.getMessageBuffer()));
	...
	/**
	 * Z-Wave controller Send Thread. Takes care of sending all messages. It
	 * uses a semaphore to synchronize communication with the receiving thread.
	 * 
	...
	private class ZWaveSendThread extends Thread {
		...
		private final Logger logger = LoggerFactory
				.getLogger(ZWaveSendThread.class);
		...
		public void run() {
			...
			try {
				while (!interrupted()) {
					// To avoid sending lots of frames when we still have input
					// frames to
					// process, we wait here until we've processed all receive
					// frames
					if (!sendAllowed.tryAcquire(1, zWaveResponseTimeout,
					...
					// If it's a battery device, it needs to be awake, or we
					// queue the frame until it is.
					if (node != null && !node.isListening()
							&& !node.isFrequentlyListening()) {
						ZWaveWakeUpCommandClass wakeUpCommandClass = (ZWaveWakeUpCommandClass) node
								.getCommandClass(CommandClass.WAKE_UP);

						// If it's a battery operated device, check if it's
						// awake or place in wake-up queue.
						if (wakeUpCommandClass != null
								&& !wakeUpCommandClass
										.processOutgoingWakeupMessage(lastSentMessage)) {
							continue;
						}
					}
					...
					// messages between steps 2 and 3 so we can get other
					// messages
					// received at step 3 that are not related to our original
					// request.
					// 4) We ultimately receive the requested message from the
					// device
					// if we're requesting such a message.
					//
					// A transaction is generally completed at the completion of
					// step 4.
					// However, for some messages, there may not be a further
					// REQUEST
					// so the transaction is terminated at step 2. This is
					// handled
					...
					// For
					// example, the requested message at step 4 may be received
					// before
					// the REQUEST at step 3. This can (I guess) occur if the
					// message to
					// the device is received by the device, but the ACK back to
					// the controller
					// is lost. The device then sends the requested data, and
					// then finally
					// the ACK is received.
					// We cover this by setting an 'AckPending' flag in the sent
					// message.
					// This needs to be cleared before the transacion is
					// completed.

					// Clear the semaphore used to acknowledge the completed
					// transaction.
					transactionCompleted.drainPermits();

					// Send the REQUEST message TO the controller
					byte[] buffer = lastSentMessage.getMessageBuffer();
					logger.debug("NODE {}: Sending REQUEST Message = {}",
					...
					// Now wait for the RESPONSE, or REQUEST message FROM the
					// controller
					// This will terminate when the transactionCompleted flag
					// gets set
					// So, this might complete on a RESPONSE if there's an error
					// (or no further REQUEST expected)
					// or it might complete on a subsequent REQUEST.
					try {
						if (!transactionCompleted.tryAcquire(1,
								zWaveResponseTimeout, TimeUnit.MILLISECONDS)) {
							timeOutCount.incrementAndGet();
							// If this is a SendData message, then we need to
							// abort
							// This should only be sent if we didn't get the
							// initial ACK!!!
							// So we need to check the ACK flag and only abort
							// if it's not set
							if (lastSentMessage.getMessageClass() == SerialMessageClass.SendData
									&& lastSentMessage.isAckPending()) {
								buffer = new SerialMessage(
										SerialMessageClass.SendDataAbort,
										SerialMessageType.Request,
										SerialMessageClass.SendData,
										SerialMessagePriority.Immediate)
										.getMessageBuffer();
								logger.debug(
										"NODE {}: Sending ABORT Message = {}",
										lastSentMessage.getMessageNode(),
										SerialMessage.bb2hex(buffer));
								try {
									synchronized (serialPort.getOutputStream()) {
							...
							// Requeue if we're ok, otherwise discard the
							// message
							if (--lastSentMessage.attempts >= 0) {
								logger.error(
										"NODE {}: Timeout while sending message. Requeueing - {} attempts left!",
										lastSentMessage.getMessageNode(),
										lastSentMessage.attempts);
								if (lastSentMessage.getMessageClass() == SerialMessageClass.SendData) {
								...
								logger.warn(
										"NODE {}: Too many retries. Discarding message: {}",
										lastSentMessage.getMessageNode(),
						...
						logger.debug(
								"NODE {}: Response processed after {}ms/{}ms.",
								lastSentMessage.getMessageNode(), responseTime,
								longestResponseTime);
						logger.trace(
								"Acquired. Transaction completed permit count -> {}",
								transactionCompleted.availablePermits());
					} catch (InterruptedException e) {
						break;
					}
	 ...
	 * @since 1.3.0
	 */
	private class ZWaveReceiveThread extends Thread implements
		...
		public void run() {
			...
			try {
				...
				// If we want to do a soft reset on the serial interfaces, do it
				// here.
				// It seems there's no response to this message, so sending it
				// through
				// 'normal' channels will cause a timeout.
				if (softReset == true) {
					try {
						synchronized (serialPort.getOutputStream()) {
							SerialMessage resetMsg = new SerialApiSoftResetMessageClass()
									.doRequest();
						...
						logger.error(
								"Error sending soft reset on initialisation: {}",
								e.getMessage());
					}
				}
				...
				while (!interrupted()) {
					...
					try {
						...
						logger.error(
								"Got I/O exception {} during receiving. exiting thread.",
								e.getLocalizedMessage());
						break;
					}
					...
					switch (nextByte) {
					case SOF:
						// Use the sendAllowed semaphore to signal that the
					...
					case NAK:
						...
						transactionCompleted.release();
						logger.trace(
								"Released. Transaction completed permit count -> {}",
								transactionCompleted.availablePermits());
						break;
					case CAN:
						CANCount++;
					...
					default:
						OOFCount++;
						logger.warn(String
								.format("Protocol error (OOF). Got 0x%02X. Sending NAK.",
										nextByte));
						sendResponse(NAK);
						break;
					}
		...
		public void run() {
			...
			if ((receiveThread != null && !receiveThread.isAlive())
				...
				try {
					...
					logger.error("Unable to restart Serial threads: {}",
							e.getLocalizedMessage());
				}
			}
		}
